# Eduardo Melo

**Desenvolvedor Backend Java · Spring Boot · React e TypeScript**

Bacharel em Sistemas de Informação, com mais de dois anos de experiência no desenvolvimento e na implantação de aplicações web para órgãos públicos. Construo APIs REST, integrações e produtos orientados a problemas reais, com atenção a segurança, testes, dados e operação.

Atualmente, aprofundo competências exigidas de profissionais Java de nível pleno por meio de projetos que exploram consistência transacional, mensageria, observabilidade, containers e nuvem. As tecnologias são adicionadas ao meu perfil à medida que passam a ter código e evidências públicas.

## Competências principais

- **Backend:** Java 17/21, Spring Boot, Spring MVC, Spring Data JPA e Spring Security
- **APIs e segurança:** REST, HTTP/JSON, Bean Validation, OAuth 2.0 e JWT
- **Dados:** PostgreSQL, SQL, JPA/Hibernate e modelagem relacional
- **Qualidade:** JUnit 5, Mockito, testes unitários e de integração, TDD e SOLID
- **Entrega:** Maven, Git/GitHub, Docker, CI/CD e Linux
- **Frontend complementar:** React, TypeScript, HTML e CSS

## Projetos em destaque

| Projeto | Problema e evidências |
| --- | --- |
| [Calculo Psicrometrico](https://play.google.com/store/apps/details?id=ifmg.grapsidroid&hl=pt_BR) | Redesenho de um aplicativo Android com mais de 10 mil downloads; a avaliação SUS evoluiu de 72,28 para 89,64. Código privado por acordo de confidencialidade. |

## Em construção

- [OrderFlow Platform](https://github.com/asmeduardo/orderflow-platform): evita venda sem estoque e inconsistências em pedidos multicanal; evoluirá com Kafka, outbox, observabilidade, Kubernetes, AWS e painel React.
- [Secure Ledger API](https://github.com/asmeduardo/secure-ledger-api): razão financeira auditável para impedir transferências duplicadas e falhas de concorrência.

Os roadmaps são públicos e deixam explícito o que já foi entregue e o que ainda está planejado.

## Experiência que orienta meus projetos

- desenvolvimento de APIs e integrações com Java e Spring Boot;
- autorização para proteger registros pertencentes a usuários diferentes;
- digitalização de processos antes mantidos em planilhas impressas;
- implantação e sustentação em Linux com Docker e CI/CD;
- construção de interfaces responsivas e integração com React.

## Contato

- [LinkedIn](https://www.linkedin.com/in/meloeduardo92)
- [E-mail](mailto:eduardo.melo01992@gmail.com)

Estou aberto a oportunidades no Brasil como desenvolvedor backend Java ou full stack com predominância de Java e frontend em React.
